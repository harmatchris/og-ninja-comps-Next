...
const JuryApp=({compId,stNum,stageId,onBack})=>{
  ...
  const athList=athletesMap?Object.values(athletesMap):[];
  const stageAthList=isPipeline?(pipelineAthIds&&pipelineAthIds.size>0?athList.filter(a=>pipelineAthIds.has(a.id)):(()=>{const _cats=pipelineStageCfg?.categories==='all'?IGN_CATS.map(c=>c.id):(Array.isArray(pipelineStageCfg?.categories)?pipelineStageCfg.categories:[]);const _cs=new Set(_cats);return athList.filter(a=>_cs.has(a.cat));})()):(athList.filter(a=>a.cat===catId));
